...
import (
	...
	tuc "github.com/kubenext/kubefun/thirdparty/unstructured"
...
func CRDEntries(ctx context.Context, prefix, namespace string, o store.Store, wantsClusterScoped bool) ([]Navigation, bool, error) {
	var list []Navigation

	loading := false

	crds, _, err := CustomResourceDefinitions(ctx, o)
	if err != nil {
	...
	for i := range crds {
		...
		objects, isLoading, err := ListCustomResources(ctx, crds[i], namespace, o, nil)
		if err != nil {
...
func CustomResourceDefinitions(ctx context.Context, o store.Store) ([]*apiextv1beta1.CustomResourceDefinition, bool, error) {
	...
	for i := range rawList.Items {
		...
		// a release so Kubefun can switch back.
		if err := tuc.DefaultUnstructuredConverter.FromUnstructured(rawList.Items[i].Object, crd); err != nil {
