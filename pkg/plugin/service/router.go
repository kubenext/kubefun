...
import "github.com/kubenext/kubefun/pkg/view/component"
...
// Request represents a path request from Kubefun. It will always be a
...
type Request struct {
	...
	// Path is path that Kubefun is requesting. It is scoped to the plugin.
	// i.e. If Kubefun wants to render /content/plugin/foo, Path will be
	// `/foo`.
	Path string
}
