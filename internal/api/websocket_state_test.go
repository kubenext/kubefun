...
func TestWebsocketState_Start(t *testing.T) {
	...
	mocks.stateManager.EXPECT().Start(gomock.Any(), gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, state kubefun.State, wsClient api.KubefunClient) {
			started <- true
		})
	s := mocks.factory()
...
type websocketStateMocks struct {
	...
	wsClient         *fake.MockKubefunClient
