...
type NavigationGeneratorFunc func(ctx context.Context, state kubefun.State, config NavigationManagerConfig) ([]navigation.Navigation, error)
...
func (n NavigationManager) Handlers() []kubefun.ClientRequestHandler {
	return nil
}

// Start starts the manager. It periodically generates navigation updates.
func (n *NavigationManager) Start(ctx context.Context, state kubefun.State, s KubefunClient) {
...
func (n *NavigationManager) runUpdate(state kubefun.State, client KubefunClient) PollerFunc {
	var previous []byte
...
func NavigationGenerator(ctx context.Context, state kubefun.State, config NavigationManagerConfig) ([]navigation.Navigation, error) {
	if state == nil {
...
func CreateNavigationEvent(sections []navigation.Navigation, defaultPath string) kubefun.Event {
	return kubefun.Event{
		Type: kubefun.EventTypeNavigation,
