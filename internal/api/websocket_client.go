...
type WebsocketClient struct {
	...
	isOpen   bool
	state    kubefun.State
	handlers map[string][]kubefun.ClientRequestHandler
	id       uuid.UUID
}

var _ KubefunClient = (*WebsocketClient)(nil)
...
func NewWebsocketClient(ctx context.Context, conn *websocket.Conn, dashConfig config.Dash, actionDispatcher ActionDispatcher, id uuid.UUID) *WebsocketClient {
	...
	client := &WebsocketClient{
		...
		conn:       conn,
		id:         id,
		send:       make(chan kubefun.Event),
		dashConfig: dashConfig,
		logger:     logger,
		handlers:   make(map[string][]kubefun.ClientRequestHandler),
	}
...
func (c *WebsocketClient) handleUnknownRequest(request websocketRequest) error {
	...
	c.Send(CreateEvent(kubefun.EventTypeUnknown, m))
	return nil
}
...
func (c *WebsocketClient) Send(ev kubefun.Event) {
	if c.isOpen {
		c.send <- ev
	}
}

func (c *WebsocketClient) RegisterHandler(handler kubefun.ClientRequestHandler) {
...
func CreateEvent(eventType kubefun.EventType, fields action.Payload) kubefun.Event {
