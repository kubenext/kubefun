...
import (
	...
	"github.com/kubenext/kubefun/internal/generator"
...
func (co *Overview) ClientRequestHandlers() []kubefun.ClientRequestHandler {
	return nil
}
...
func (co *Overview) Navigation(ctx context.Context, namespace, root string) ([]navigation.Navigation, error) {
	...
	nf := kubefun.NewNavigationFactory(namespace, root, objectStore, navigationEntries)
...
func (co *Overview) Generators() []kubefun.Generator {
...
func (co *Overview) ActionPaths() map[string]action.DispatcherFunc {
	dispatchers := action.Dispatchers{
		kubefun.NewDeploymentConfigurationEditor(co.logger, co.dashConfig.ObjectStore()),
		kubefun.NewContainerEditor(co.dashConfig.ObjectStore()),
		kubefun.NewServiceConfigurationEditor(co.dashConfig.ObjectStore()),
	}
