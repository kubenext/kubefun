...
import (
	...
	ius "github.com/kubenext/kubefun/internal/util/strings"
...
func CustomResourceListHandler(crdObject *unstructured.Unstructured, resources *unstructured.UnstructuredList, version string, linkGenerator link.Interface) (component.Component, error) {
	...
	crd, err := kubefun.NewCustomResourceDefinition(crdObject)
	if err != nil {
	...
	if len(resources.Items) > 0 {
		...
		for _, column := range version.PrinterColumns {
			name := column.Name
			if ius.Contains(column.Name, []string{"Name", "Labels", "Age"}) {
...
func printCustomColumn(m interface{}, column kubefun.CustomResourceDefinitionPrinterColumn) (string, error) {
...
func crdVersion(crd, cr *unstructured.Unstructured) (kubefun.CustomResourceDefinitionVersion, error) {
	if crd == nil {
		return kubefun.CustomResourceDefinitionVersion{}, fmt.Errorf("custom resource definition is nil")
	}

	if cr == nil {
		return kubefun.CustomResourceDefinitionVersion{}, fmt.Errorf("custom resource is nil")
	}

	kubefunCRD, err := kubefun.NewCustomResourceDefinition(crd)
	if err != nil {
		return kubefun.CustomResourceDefinitionVersion{}, fmt.Errorf("create kubefun CRD: %w", err)
	}

	crdVersion, err := kubefunCRD.Version(cr.GroupVersionKind().Version)
	if err != nil {
		return kubefun.CustomResourceDefinitionVersion{}, fmt.Errorf("get version for custom resource: %w", err)
	}
